...
1.  **Application:** A Python web application built using the **FastAPI** framework. It exposes RESTful endpoints for managing user birthday information. Code is located in the `birthday-app/` directory.
...
5.  **Connectivity:** The application connects to the Cloud SQL database securely via the **Cloud SQL Auth Proxy** running as a sidecar container, which uses **Workload Identity** and passwordless **IAM Database Authentication**.

6.  **Exposure:** The application is exposed externally via the **Kubernetes Gateway API**, managed by GKE's native Gateway controller (Google Cloud Load Balancer).
...
8.  **CI/CD:** Automated build, test , infrastructure deployment, and application deployment are handled by **GitHub Actions** workflows defined in `.github/workflows/`.


## Design Decisions

* **GCP & GKE:** Chosen for container management and easy integration with other GCP services (Cloud SQL, Container Registry, IAM), and scalability. A public cluster was chosen to simplify CI/CD runner access for this exercise.

* **Cloud SQL (Postgres, Private IP, IAM Auth):** Provides a managed, reliable database service. Using Private IP and the Proxy sidecar with IAM authentication significantly enhances security. Regional configuration provides high availability.
...
* **Kubernetes Gateway API:** Standard Kubernetes API for configuring L7 ingress and traffic routing, offering more features and flexibility than older Ingress resource.
...
1. **The service account used by CI/CD Pipeline will need the following permission to create infrastructure and dploy the application:**
...
3.  **Application (`application.yaml`):**
    ...
    * Builds and push a multi-platform Docker image to Artifact Registry (tagged with Git SHA).
...
**Manual Pre-step:** Before the first deployment (or if the database is recreated), the necessary permissions need to be granted *inside* PostgreSQL database for the application's IAM user. Connect to the Cloud SQL instance, using console or glcoud command, reset the super user password if needed. make sure the user name is correct as created by terraform.
...
Kubernetes manifests defining the deployment, service, migration job, gateway, etc., are located in `birthday-app/k8s/`.
...
* **Logging:** The FastAPI application is configured to output logs to standard output (`stdout`). In the GKE environment, these logs are automatically collected by the integrated logging agent and are viewable within **Google Cloud Logging**. The current format is plain text, but structured (JSON) logging could be implemented for enhanced querying.
* **Monitoring:** Basic GKE cluster and node metrics (CPU, memory, disk, network) are automatically collected and available in **Google Cloud Monitoring**. Application-level metrics (e.g., request latency, error rates specific to endpoints) are not currently implemented but could be added using Prometheus client libraries or OpenTelemetry.

## Testing

Unit and integration tests for the FastAPI application are located in `birthday-app/tests/`. They can be run locally using Docker Compose as described in the [birthday-app/README.md](./birthday-app/Readme.md).
